...
export default function DetailScreen({ plant: p, weekdays, goBack }: Props) {
  ...
  const openDatePicker = (rawDate: string) => {
    Analytics.click({ log_name: 'click_detail_edit_first_water_open', plant_id: p.id });
  ...
  const saveEdit = () => {
    ...
    const orig = p.editable.initialData || p.editable;
    const changed: string[] = [];
    if ((editName.trim() || p.editable.name) !== orig.name) changed.push('name');
    if (editSpeciesName !== orig.speciesName) changed.push('species_name');
    if (editSci !== orig.sci) changed.push('sci');
    if (editType !== orig.type) changed.push('type');
    if (editColor !== orig.color) changed.push('color');
    if (waterIntervalDays.spring !== orig.waterIntervalDays.spring || waterIntervalDays.summer !== orig.waterIntervalDays.summer || waterIntervalDays.autumn !== orig.waterIntervalDays.autumn || waterIntervalDays.winter !== orig.waterIntervalDays.winter) changed.push('water_interval');
    if (editLight !== orig.light) changed.push('light');
    if (editTemp !== orig.temp) changed.push('temp');
    Analytics.click({ log_name: 'click_detail_edit_save', plant_id: p.id, fields_changed: changed.join(','), changed_count: changed.length });
  ...
  return (
    <div className="scrl" style={{ paddingTop: 'max(54px, calc(38px + var(--safe-top)))', paddingBottom: 32, overflowY: 'auto', minHeight: '100vh' }}>
            ...
            {p.history.map((h, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 13, padding: '10px 4px', borderBottom: '1.5px dashed var(--line)' }}>
                ...
                {h.isFirst ? (
                  ...
                  <button
                    onClick={() => { Analytics.click({ log_name: 'click_detail_cancel_water', plant_id: p.id }); p.onCancelWatering(); }}
      ...
      <div style={{ margin: '28px 20px 0', paddingTop: 20, borderTop: '1.5px dashed var(--line)', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {p.archived ? (
          <button
            ...
            style={{ width: '100%', padding: '13px', border: '2px solid var(--ink)', borderRadius: 18, background: 'transparent', color: 'var(--ink)', fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
          >
            다시 키우기
          </button>
        ) : (
      ...
      {deleteConfirm && (
        <>
          <div onClick={() => setDeleteConfirm(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(35,31,24,.32)', zIndex: 50 }} />
          <div style={{ position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: 51, background: 'var(--paper)', borderRadius: '30px 30px 0 0', border: '2.5px solid var(--ink)', borderBottom: 'none', padding: '22px 22px calc(30px + var(--safe-bottom))', animation: 'popIn .26s ease' }}>
            <div style={{ width: 44, height: 5, borderRadius: 3, background: 'var(--line)', margin: '0 auto 18px' }} />
            <div style={{ fontFamily: 'KJD, sans-serif', fontSize: 22, fontWeight: 700, marginBottom: 8 }}>정말 삭제할까요?</div>
            <div style={{ fontSize: 14, color: 'var(--soft)', lineHeight: 1.55, marginBottom: 24 }}>
              {p.name}의 기록이 모두 삭제되며,<br />복구할 수 없어요.
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                onClick={() => setDeleteConfirm(false)}
                style={{ flex: 1, padding: '13px', border: '2px solid var(--line)', borderRadius: 18, background: 'transparent', color: 'var(--soft)', fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
              >
                취소
              </button>
              <button
                onClick={p.onDelete}
                style={{ flex: 1, padding: '13px', border: '2px solid #CC6B52', borderRadius: 18, background: '#CC6B52', color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
              >
                삭제하기
              </button>
            </div>
      ...
      {pickerOpen && (
        <>
          ...
          <div style={{ position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: 51, background: 'var(--paper)', borderRadius: '30px 30px 0 0', border: '2.5px solid var(--ink)', borderBottom: 'none', padding: '14px 22px calc(30px + var(--safe-bottom))', animation: 'popIn .26s ease' }}>
            ...
            <div
              onClick={() => { if (pickerDate) { Analytics.click({ log_name: 'click_detail_edit_first_water_save', plant_id: p.id, new_date: pickerDate }); p.onEditFirstWatering(pickerDate); setPickerOpen(false); } }}
