...
export default function CalendarScreen({ monthLabel, weeks, chips, upcoming, weekdays, onPrevMonth, onNextMonth }: Props) {
  return (
    <div className="scrl" style={{ padding: 'max(62px, calc(44px + var(--safe-top))) 18px 32px', overflowY: 'auto' }}>
      {/* 월 이동 */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        ...
        <div style={{ fontFamily: 'Caveat, cursive', fontSize: 34, fontWeight: 700, lineHeight: 1 }}>{monthLabel}</div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginBottom: 14 }}>
        {chips.map(c => (
          <button key={c.id} onClick={c.onToggle} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 11px 6px', borderRadius: 14, border: `2px solid ${c.on ? c.color : 'var(--line)'}`, background: c.on ? `${c.color}1e` : 'transparent', cursor: 'pointer', fontSize: 13, fontWeight: 700, color: c.on ? 'var(--ink)' : 'var(--soft)', fontFamily: 'inherit' }}>
      ...
      {/* 달력 그리드 */}
      <div style={{ border: '2px solid var(--ink)', borderRadius: 22, padding: '12px 10px 14px', background: 'var(--paper)' }}>
        {/* 요일 헤더 */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', marginBottom: 4 }}>
          {weekdays.map(w => (
            <div key={w.label} style={{ textAlign: 'center', fontSize: 11.5, fontWeight: 700, color: w.col, paddingBottom: 4 }}>{w.label}</div>
          ))}
        </div>
        {/* 날짜 셀 */}
        {weeks.map((week, wi) => (
          <div key={wi} style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)' }}>
            {week.map((d, di) => (
              <div key={di} style={{ aspectRatio: '1/1.08', display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 5, position: 'relative', background: d.bandBg, borderRadius: 9 }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 600, color: d.numCol, zIndex: 1, lineHeight: 1.5 }}>{d.day}</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, margin: '12px 4px 0', fontSize: 11.5, color: 'var(--soft)' }}>
        ...
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
          <span style={{ width: 14, height: 9, borderRadius: 3, background: 'rgba(133,123,105,.22)' }} />물주기 구간
        </span>
      </div>
      ...
      {upcoming.length > 0 && (
        <div style={{ marginTop: 20 }}>
          <div style={{ fontFamily: 'Caveat, cursive', fontSize: 26, fontWeight: 700, marginBottom: 10 }}>다가오는 물주기</div>
          {upcoming.map((u, i) => (
            <div key={i} onClick={u.onOpen} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', marginBottom: 9, border: '2px solid var(--ink)', borderRadius: '17px 19px 16px 18px', cursor: 'pointer' }}>
              <span style={{ width: 11, height: 11, borderRadius: '50%', background: u.color, flexShrink: 0 }} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 15.5, fontWeight: 700 }}>{u.name}</div>
