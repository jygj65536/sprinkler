...
→ **권장**: PoC는 mock JSON으로 시작, 이후 공공데이터 API 연동.
...
→ **권장**: PoC는 Native Storage만 사용. 서버는 검수 후 필요 시 추가.
...
→ **권장**: PoC는 **앱 접속 시 배너 알림만** 구현. 실제 푸시는 서버 구축 후 2차.
...
→ **권장**: 로그인은 포함하되 기능을 block하지 않는 형태로 최소 구현.
