...
`src/` 하위 파일을 수정할 때마다 반드시 다음을 지킨다:

1. 수정한 로직에 대한 테스트를 `src/__tests__/`에 추가하거나 기존 테스트를 업데이트한다.
2. 작업 완료 전에 `npm run test`를 실행해 전체 테스트가 통과하는지 확인한다.
3. 실패하는 테스트가 있으면 완료로 보고하기 전에 수정한다.
